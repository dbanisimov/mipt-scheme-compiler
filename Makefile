...
# Target directories
TARGET_DIRS:= UnitTest Interpreter Lowering ir Graph Utils
...
# Debug targets
debug: utestd interpreterd loweringd
utestd: gen utestd_link
interpreterd: gen interpreterd_link
loweringd: gen loweringd_link
...
interpreterd_link: $(call FILTER,Frontend Interpreter, $(DEBUG_OBJS)) 
	@echo [linking] $(BIN_DIR)/interpreterd
	@$(MKDIR) -p $(BIN_DIR)
	@$(CXX) $(DEBUG_LIB_FLAGS) -o $(BIN_DIR)/interpreterd $(call FILTER,Frontend Interpreter,$(DEBUG_OBJS)) $(DEBUG_LIB_DIRS) $(DEBUG_LIBS)

interpreter_link: $(call FILTER,Frontend Interpreter,$(RELEASE_OBJS))
	@echo [linking] $(BIN_DIR)/interpreter
	@$(MKDIR) -p $(BIN_DIR)
	@$(CXX) $(RELEASE_LIB_FLAGS) -o $(BIN_DIR)/interpreter $(call FILTER,Frontend Interpreter,$(RELEASE_OBJS)) $(RELEASE_LIB_DIRS) $(RELEASE_LIBS)

loweringd_link: $(call FILTER,Utils Graph Frontend ir/ Lowering, $(DEBUG_OBJS)) 
	@echo [linking] $(BIN_DIR)/loweringd
	@$(MKDIR) -p $(BIN_DIR)
	@$(CXX) $(DEBUG_LIB_FLAGS) -o $(BIN_DIR)/loweringd $(call FILTER,Utils Graph Frontend ir/ Lowering, $(DEBUG_OBJS)) $(DEBUG_LIB_DIRS) $(DEBUG_LIBS) $(BOOST_UTF_LIB)
